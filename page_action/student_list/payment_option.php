...
if(isset($_POST['payment_option_overview'])){
    ...
    <?php
  	 ...
  	 make_dive_payment("Total Fee (AUD $)",$info['total_fee'],"fa fa-usd");
  	 make_dive_payment("Total Pay (AUD $)",$info['total_pay'],"fa fa-usd");
  	 make_dive_payment("Total Due (AUD $)",$info['total_due'],"fa fa-usd");
    echo "</div>";
...
if(isset($_POST['program_overview_send_sms'])){
	...
	foreach ($info as $key => $value) {
		...
		if($p_status==$status || $status=="All"){
			...
			if($type==1){
				$msg.="Your admission fee AUD $".$fee." for '$program_name'";
			}
			else{
				$msg.="Your monthly fee AUD $".$fee." for '$month_name-$year' in '$program_name'";
			}
			...
			if($p_status=="Paid"){
				...
				$msg.=" is not completed. Your due amount AUD $".$due.". Please pay your due amount$within.";
			}
